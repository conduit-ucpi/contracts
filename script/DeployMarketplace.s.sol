...
/**
 * Deploys MarketplaceEscrow — step 3 of the §3.4 launch order, AFTER the escrow
 * implementation and factory are live.
 *
 * ⚠️ RUN THIS ONCE, DELIBERATELY. It is deliberately NOT part of the tag-triggered
 *    DeploymentScript: that runs on every release tag, and redeploying the marketplace
 *    would orphan every live offer and every unsettled holdback held by the old one
 *    (their funds are recoverable — exits are never pausable — but every LP would have
 *    to be told to withdraw from an address the UI no longer points at).
 *
...
contract DeployMarketplace is Script {
    ...
    function run() external {
        ...
        MarketplaceEscrow marketplace =
            new MarketplaceEscrow(implementation, feeBps, minOfferBps, offerDuration, owner);
        vm.stopBroadcast();

        console.log("Marketplace deployed at:", address(marketplace));

        // §16 phase 6.5: recompute the codehash against the FINAL implementation address.
        // Read it back off the deployed contract so the log proves what it will enforce.
        console.log("=================================================");
        console.log("EXPECTED_ESCROW_CODEHASH (record this):");
        console.logBytes32(marketplace.EXPECTED_ESCROW_CODEHASH());
        console.log("=================================================");
