...
3. **Live seller:** an offer is acceptable only while `escrow.recipient() == offer.seller`, only by that address, **and only while `escrow.recipientNonce() == offer.sellerNonce`**. Any recipient change makes every existing offer on that escrow permanently stale (withdrawable, never acceptable). Payment always goes to `msg.sender == offer.seller == recipient()` — a stale snapshot can never be paid.

   The nonce is what makes "permanently" true. The address comparison alone is **reversible**: a seller who rotates their payout wallet and rotates back mutes every offer on the escrow and then revives them all. `recipientNonce` is a monotonic counter on the escrow, bumped by every recipient move (`changeRecipient` and `transferRecipientFrom` alike) and never reset, so a move can be observed as one-way by anyone holding a snapshot. It is deliberately distinct from `hasBeenSold`, which means "a sale happened" and gates the one-reserve rule — a wallet rotation is not a sale and must not forfeit the right to sell with a reserve later.
...
11. **Exits are open, and safe because every exit condition is one-way.** `withdraw` is callable by anyone and always pays that offer's `lp`. This is safe only because each condition that makes an offer withdrawable is irreversible — the clock only advances; a dispute resolves to claimed and never back to funded; `isClaimed` and `hasBeenSold` are absorbing; and staleness is measured by the monotonic `recipientNonce` (invariant 3), not by a comparison that can flip back. An offer that is withdrawable can therefore never become acceptable again, so no caller can destroy anything the LP still valued, and none can settle a vault in front of the seller's own `accept`. **Any future condition added to the withdrawability matrix must be one-way, or `withdraw` must be re-closed to the LP.**
...
4a. **The holdback rule needs no re-check at acceptance — the staleness test subsumes it.** `hasBeenSold` can flip between an offer's creation and its acceptance, and this offer's holdback was validated against the value at *creation* time. But the only writer of `hasBeenSold` is `transferRecipientFrom`, which moves the recipient and so bumps `recipientNonce`; the offer is therefore already permanently stale (invariant 3) and step 3's check has rejected it. A `holdback > 0 && hasBeenSold` branch here could never fire, so the implementation does not carry one. **The danger it guarded is real and is still guarded:** Two LPs may both bid with a holdback while an escrow is unsold (both legal); the seller accepts the first, recording the one permitted reserve; if the position then returns to that same seller — a direct `changeRecipient` back, or the seller buying their own cashflow back as an LP, which they may do because after the first sale they are no longer the recipient — the second offer becomes live again and accepting it would **overwrite `holdbacks[escrow]` while still adding to `totalHoldbacks`**, stranding the first reserve permanently. See §0.4c H-1.
...
1. Key = (escrowContract, `msg.sender`). Withdrawable iff:
   ...
   - `status == OPEN &&` any of:
     ...
     - `escrow.recipientNonce() != offer.sellerNonce` (stale — the recipient MOVED; see §5.2.11), or
     ...
     (A reserve-bearing offer on an escrow that has since been sold needs no term of its own: the sale moved the recipient, so the staleness test above already reports it withdrawable.)
   The matrix is exposed as `isWithdrawable()` — `withdraw()` branches on it, and §15.2's indexer reads it rather than reimplementing the conditions. Otherwise revert `NothingToWithdraw`. No withdrawable offer is ever simultaneously acceptable (acceptance rejects every one of these conditions), so there is no accept/withdraw race.
   **Callable by anyone** (§5.2.11): the destination is the offer's `lp`, fixed at creation; the amount is fixed by status; and every condition above is read from the escrow or the clock. A caller chooses nothing, so a stranger can only return the LP's capital at their own gas expense — which lets a keeper or the platform close out expired offers for LPs who are not watching.
